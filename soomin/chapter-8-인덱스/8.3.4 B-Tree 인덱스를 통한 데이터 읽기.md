...
EXPLAIN SELECT phone FROM employee WHERE name = '김수한무';
...
```sql
-- 인덱스 (birth_date, gender)
-- 가능: 인덱스 컬럼만 사용
SELECT gender FROM employee WHERE birth_date > '1998-01-01';

-- 불가능: 인덱스에 없는 컬럼 사용
SELECT phone FROM employee WHERE birth_date > '1998-01-01';
```
...
```sql
-- 인덱스: ix_gender_birthdate (birth_date, gender)
...
속도 순 오름 차순임. (루스 인덱스 스캔이 제일 빠름)

```java
루스 인덱스 스캔 < 인덱스 레인지 스캔 < 인덱스 풀 스캔 < 인덱스 스킵 스캔 < 테이블 풀 스캔
```
...
- **커버링 인덱스 활용**: 데이터 페이지 접근 완전 회피
- **인덱스 레인지 스캔 최적화**: 스캔 범위를 최대한 좁히기

#### 작업 범위 결정 조건 vs 체크 조건

```sql
-- 좋은 예: 두 조건 모두 작업 범위 결정
-- INDEX (dept_no, emp_no)
WHERE dept_no = 'd002' AND emp_no >= 10114

-- 나쁜 예: dept_no는 단순 체크 조건
-- INDEX (emp_no, dept_no)
WHERE emp_no >= 10114 AND dept_no = 'd002'  -- 비효율적
```
...
3. **ORDER BY, GROUP BY** 에 사용되는 컬럼 고려

#### 커버링 인덱스 고려사항

- SELECT, WHERE, ORDER BY, GROUP BY에 사용되는 모든 컬럼 포함
- 인덱스 크기 vs 성능 향상 효과 비교 검토

### 4. 실행 계획 해석

| type  | 의미                             | 성능 |
| ----- | -------------------------------- | ---- |
| const | 기본키나 유니크 키로 단 1건 조회 | 최고 |
| range | 인덱스 레인지 스캔               | 좋음 |
| index | 인덱스 풀 스캔                   | 보통 |
| ALL   | 테이블 풀 스캔                   | 나쁨 |
...
- **Using index for group-by**: 루스 인덱스 스캔 사용
- **Using index for skip scan**: 인덱스 스킵 스캔 사용
- **Using where**: WHERE 조건으로 필터링
- **Using filesort**: 별도 정렬 작업 필요

> Using이면 권장, Force면 무조건 사용
